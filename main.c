...
int num_training_ex;
float loss=0;//an index to save full_cost to create a file
FILE *losscurve;
...
//whether the architecture created successfully or not
int init()
{
...
void train_neural_net(void)
{
    int i;
    int train_times=0;
    losscurve=fopen("losscurve.csv","w+");

    //write a csv file to draw the loss curve
    if(losscurve==NULL)
        printf("File could not be open.\n");
    else
        fprintf(losscurve,"times,loss\n");

    // Gradient Descent
    for(train_times=0;train_times<20000;train_times++)
    {
        for(i=0;i<num_training_ex;i++)
        {
            feed_input(i);
...
void update_weights(void)
{
    ...
    for(i=0;i<num_layers-1;i++)
    {
        for(j=0;j<num_neurons[i];j++)
        {
            ...
            {
                // Update Weights
                lay[i].neu[j].out_weights[k] = (lay[i].neu[j].out_weights[k]) - (learningrate * lay[i].neu[j].dw[k]);
            }
            
            // Update Bias
            lay[i].neu[j].bias = lay[i].neu[j].bias - (learningrate * lay[i].neu[j].dbias);
        }
    }   
}
...
// Compute Total Cost(loss function)
